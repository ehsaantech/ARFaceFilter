...
function interpolate_detectedStates() {
  ...
  var mix_states = function (sa, sb, t) {
  ...
  GIF.detectedStates = GIF.detectedStates.map(function (ds, dsi) {
    ...
    return mix_states(
      get_dState(dsi - distBefore),
      get_dState(dsi + distAfter),
      t
    );
  });
...
function build_gifFrameMask(detectState, frameIndex) {
  ...
  GIF.potFaceCutTextureSizePx = Math.pow(
    2,
...
function update_positionUserCropCanvas(frameIndex) {
  ...
  var domCanvasNotDetected =
    document.getElementsByClassName("canvasNotDetected");
...
function draw_render(detectState) {
  ...
  var xn =
    detectState.x * 0.5 +
    0.5 +
    s * SETTINGS.gifMaskOffset[0] * Math.sin(detectState.ry); //normalized x position
  ...
  GL.bindTexture(
    GL.TEXTURE_2D,
    GIF.hueTextures[GIF.animation.currentFrameIndex]
  );
...
function callbackTrack(detectState) {
  switch (STATE) {
    ...
    case STATES.DETECTUSERFACE:
      ...
      var currentFrameIndex = Math.floor(
        (GIF.frames.length *
          (currentTimestamp - GIF.animation.startTimestamp)) /
          GIF.info.duration
      );
      ...
      if (currentFrameIndex !== GIF.animation.currentFrameIndex) {
        ...
        if (GIF.frameMasks[currentFrameIndex]) {
          GIF.canvasMaskCtx.clearRect(
            0,
            0,
            GIF.canvasMask.width,
            GIF.canvasMask.height
          );
